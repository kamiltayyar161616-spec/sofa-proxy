...
def compute_expected_scores(hg, ag, hv, av, odds_implied):
    ...
    if has_stats:
        ...
        # ── Madde 3: Pace & Efficiency ayarı ──────────────────────
        # Pace proxy = (scored+conceded)/2 ≈ 108 bandında
        # Referans da aynı ölçekte: LEAGUE_AVG/2 yerine LEAGUE_AVG kullanıyoruz
        # Çünkü pace = tek takımın pozisyon başına skoru değil, maç ortalaması
        home_pace = hg.get("pace", LEAGUE_AVG / 2)  # ~54-60 arası
        away_pace = ag.get("pace", LEAGUE_AVG / 2)
        avg_pace  = (home_pace + away_pace) / 2
        # Referans: LEAGUE_AVG/2 (54) — fark tipik olarak ±5-10, katsayı küçük tutulur
        pace_adj  = max(-5.0, min(5.0, (avg_pace - (LEAGUE_AVG / 2)) * 0.08))

        # Efficiency: 100 pozisyon başına üretim — sadece çok belirgin farkta etki
        home_eff = hg.get("efficiency", 100.0)
        away_eff = ag.get("efficiency", 100.0)
        avg_eff  = (home_eff + away_eff) / 2
        eff_adj  = 1.5 if avg_eff > 108.0 else (-1.5 if avg_eff < 95.0 else 0.0)

        # Bahisçi line'ı varsa pace/eff sadece istatistik parçasına etki eder
        # final_total zaten %80+ bahisçi ağırlıklı olacak, ek etki minimal
        if not odds_implied.get("market_ou_center"):
            stat_total    = stat_total    + pace_adj + eff_adj
            stat_iy_total = stat_iy_total + (pace_adj + eff_adj) * IY_RATIO
        # ──────────────────────────────────────────────────────────

        n_total     = hg["n"] + ag["n"]
        # İstatistik max %50 ağırlık; bahisçi line'ı varsa %20 ile sınırla
        has_market = odds_implied.get("market_ou_center") is not None
        stat_cap   = 0.20 if has_market else 0.50
    ...
    if market_center is not None:
        ...
        if has_stats and stat_weight > 0:
            # İstatistiğin totale katkısı max %15
            stat_contribution = min(0.15, stat_weight * 0.2)
        ...
        # Oranlardan beklenti var ama merkez yok (az çizgi)
        if has_stats and stat_weight > 0:
        ...
        # Sadece istatistik
        if has_stats:
            final_total  = stat_total
            final_spread = stat_spread
        else:
            final_total  = LEAGUE_AVG
...
def run_value_hunting(home_general, home_venue, away_general, away_venue, market_odds=None):
    ...
    # Madde 1: Son 6 maç, ağırlıklı (son 3'e %20 fazla) — normalize_stats içinde uygulanır
