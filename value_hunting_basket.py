...
Sadece takımların gerçek maç istatistiklerini kullanır:
  - Son 6 genel maç
  - Son 6 iç saha maçı
  - Son 6 dış saha maçı

Ekstra sabit, lig ortalaması, pace, efficiency YOK.
Veri yoksa fallback döner.
"""
...
PAYOUT          = 0.92
VALUE_THRESHOLD = 3.0
RECENT_BOOST    = 1.20   # Son 3 maça %20 fazla ağırlık
...
def _w(n):
    """Son 3 maça RECENT_BOOST, öncesine 1.0"""
...
# ════════════════════════════════════════════════════════════════
# İSTATİSTİK — sadece atılan/yenilen/toplam/IY
# ════════════════════════════════════════════════════════════════

def _parse(matches, iy_from_quarters=False):
    """Son 6 maçtan ağırlıklı ortalama çıkar."""
    n = len(matches)
    if n == 0:
        return None
    ws = _w(n)
    ...
    # IY: Q1+Q2 varsa kullan, yoksa iy_scored
    if iy_from_quarters:
        iy_s_list, iy_c_list, iy_ws = [], [], []
        for i, m in enumerate(matches):
            ...
            if s > 0:
                iy_s_list.append(s); iy_c_list.append(c); iy_ws.append(ws[i])
        avg_iy_s = _wavg(iy_s_list, iy_ws) if iy_s_list else avg_sc * 0.47
        avg_iy_c = _wavg(iy_c_list, iy_ws) if iy_c_list else avg_co * 0.47
    else:
    ...
    return {
        "avg_sc": avg_sc, "avg_co": avg_co, "avg_tot": avg_tot,
...
# ════════════════════════════════════════════════════════════════
# BEKLENEN SKOR — salt istatistik
# ════════════════════════════════════════════════════════════════

def _expected(hg, ag, hv, av):
    """
    Ev takımı: venue istatistiği %60 + genel %40
    Deplasman: venue istatistiği %60 + genel %40
    Beklenen skor = (saldırı + rakip savunma) / 2
    """
    if hv and hv["n"] >= 2:
        h_att = hv["avg_sc"] * 0.6 + hg["avg_sc"] * 0.4
        h_def = hv["avg_co"] * 0.6 + hg["avg_co"] * 0.4
        h_iy  = hv["avg_iy_s"] * 0.6 + hg["avg_iy_s"] * 0.4
    else:
        h_att = hg["avg_sc"]
        h_def = hg["avg_co"]
        h_iy  = hg["avg_iy_s"]
    ...
    if av and av["n"] >= 2:
        a_att = av["avg_sc"] * 0.6 + ag["avg_sc"] * 0.4
        a_def = av["avg_co"] * 0.6 + ag["avg_co"] * 0.4
        a_iy  = av["avg_iy_s"] * 0.6 + ag["avg_iy_s"] * 0.4
    else:
        a_att = ag["avg_sc"]
        a_def = ag["avg_co"]
        a_iy  = ag["avg_iy_s"]

    # Beklenen skor: her takımın saldırısı + rakibin savunması ortalaması
    exp_home = (h_att + a_def) / 2 + HOME_ADV / 2
    exp_away = (a_att + h_def) / 2 - HOME_ADV / 2

    exp_iy_home = (h_iy + (av["avg_iy_c"] if av and av["n"] >= 2 else ag["avg_iy_c"])) / 2
    exp_iy_away = (a_iy + (hv["avg_iy_c"] if hv and hv["n"] >= 2 else hg["avg_iy_c"])) / 2
    ...
    return {
        ...
        "exp_iy_total": round(exp_iy_home + exp_iy_away, 1),
        "spread":       round(exp_home - exp_away, 1),
    }


# ════════════════════════════════════════════════════════════════
# STD — takım verilerinden
# ════════════════════════════════════════════════════════════════

def _std(var):
    return max(8.0, math.sqrt(var)) if var else 12.0
...
def _ou_auto(exp_total, std):
    """Bahisçi line'ı yoksa exp_total etrafında 7 barem üret."""
...
def run_value_hunting(home_general, home_venue, away_general, away_venue,
    ...
    # Beklenen skorlar
    exp    = _expected(hg, ag, hv, av)
    exp_iy = _expected(hg_iy, ag_iy, hv_iy, av_iy)
    ...
    # STD — takımların varyansından
    std_home = _std(hg["var"])
    std_away = _std(ag["var"])
    std_diff = max(10.0, math.sqrt(std_home**2 + std_away**2))
    # Toplam STD: maç bazlı gerçek std ~ %5-6 oranında toplam
    std_total = max(10.0, min(std_diff, exp_total * 0.055))
    std_iy    = max(6.0,  exp_iy_total * 0.055)
