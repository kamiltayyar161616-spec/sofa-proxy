...
Sadece gerçek maç istatistiklerini kullanır:
  - Son 6 genel maç
  - Son 6 iç saha
  - Son 6 dış saha

TOPLAM TAHMİNİ: Takımların avg_total (gerçek maç toplam ortalaması) kullanılır.
Saldırı+savunma kombinasyonu toplamı şişiriyordu — düzeltildi.
"""
...
def _parse(matches, iy_from_quarters=False):
    ...
    avg_tot = _wavg(totals,   ws)   # ← Gerçek maç toplam ortalaması
    var     = _wavg([(x - avg_sc)**2 for x in scored], ws) if n > 1 else 100.0
    ...
    # IY toplam — sadece iy_scored kullan (atılan), maç toplamı için değil
    # avg_iy_tot: bu takımın maçlarındaki ortalama IY TOPLAM skoru
    iy_tots = []
    iy_tot_ws = []
    for i, m in enumerate(matches):
        ...
        if iy_s > 0 and iy_c > 0:
            iy_tots.append(iy_s + iy_c)   # bu takımın maçındaki gerçek IY toplamı
            iy_tot_ws.append(ws[i])
    avg_iy_tot = _wavg(iy_tots, iy_tot_ws) if iy_tots else avg_tot * 0.47

    return {
        "avg_sc": avg_sc, "avg_co": avg_co,
        "avg_tot": avg_tot,         # gerçek toplam ortalaması
        "avg_iy_tot": avg_iy_tot,   # gerçek IY toplam ortalaması
        "avg_iy_s": avg_iy_s, "avg_iy_c": avg_iy_c,
        "win_rate": wins / n, "var": var, "n": n,
    }
...
def _expected(hg, ag, hv, av, hg_iy, ag_iy, hv_iy, av_iy):
    """
    MS TOPLAM: Her iki takımın gerçek avg_total ortalaması.
    Bu, saldırı+savunma kombinasyonundan çok daha gerçekçi.

    MS SPREAD: Saldırı farkından hesapla (kim daha fazla atar).

    IY TOPLAM: Gerçek avg_iy_tot ortalaması.
    """

    # ── Toplam tahmin ─────────────────────────────────────────
    # Ev takımının maçlarındaki gerçek toplam ortalaması
    h_tot = (hv["avg_tot"] * 0.6 + hg["avg_tot"] * 0.4) if (hv and hv["n"] >= 2) else hg["avg_tot"]
    # Deplasman takımının maçlarındaki gerçek toplam ortalaması
    a_tot = (av["avg_tot"] * 0.6 + ag["avg_tot"] * 0.4) if (av and av["n"] >= 2) else ag["avg_tot"]
    # İkisinin ortalaması = beklenen maç toplamı
    exp_total = (h_tot + a_tot) / 2

    # ── Spread tahmin ─────────────────────────────────────────
    # Ev saldırısı - Deplasman savunması
    ...
    exp_home_raw = (h_att + a_def) / 2 + HOME_ADV / 2
    exp_away_raw = (a_att + h_def) / 2 - HOME_ADV / 2
    spread_raw   = exp_home_raw - exp_away_raw

    # Toplam sabitken spread'den ev/deplasman dağılımı
    exp_home = (exp_total + spread_raw) / 2
    exp_away = (exp_total - spread_raw) / 2

    # ── IY toplam ─────────────────────────────────────────────
    # avg_iy_tot zaten maç toplamını içeriyor, ikiye bölme
    h_iy_tot = (hv_iy["avg_iy_tot"] * 0.6 + hg_iy["avg_iy_tot"] * 0.4) if (hv_iy and hv_iy["n"] >= 2) else hg_iy["avg_iy_tot"]
    a_iy_tot = (av_iy["avg_iy_tot"] * 0.6 + ag_iy["avg_iy_tot"] * 0.4) if (av_iy and av_iy["n"] >= 2) else ag_iy["avg_iy_tot"]
    # İkisi de aynı maçın toplamını temsil ediyor, ortalamasını al
    exp_iy_total = (h_iy_tot + a_iy_tot) / 2
    ...
    iy_home_raw = (h_iy_att + a_iy_def) / 2
    iy_away_raw = (a_iy_att + h_iy_def) / 2
    iy_spread   = iy_home_raw - iy_away_raw
...
def _ou_auto(exp_total, std):
    base  = round(exp_total / 2.5) * 2.5
    lines = []
    for off in [-12.5, -7.5, -2.5, 0, 2.5, 7.5, 12.5]:
        line = base + off
        if line <= 0: continue
        p_ov = round(max(1.0, min(99.0, (1 - normal_cdf(line, exp_total, std)) * 100)), 1)
        lines.append({
...
def run_value_hunting(home_general, home_venue, away_general, away_venue,
    ...
    std_home  = max(8.0, math.sqrt(hg["var"]))
    std_away  = max(8.0, math.sqrt(ag["var"]))
    std_diff  = max(10.0, math.sqrt(std_home**2 + std_away**2))
    # Toplam STD: gerçek total varyansını kullan (maçların toplam dağılımı)
    # ~%5 oranı gerçekçi (NBA: 225*0.05=11.2, Euro: 160*0.05=8)
    std_total = max(9.0, min(std_diff * 0.85, exp_total * 0.05))
    std_iy    = max(5.0, exp_iy_total * 0.05)
    ...
    # IY maç sonucu
    std_iy_diff = max(6.0, std_diff * 0.47)
