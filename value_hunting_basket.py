...
def compute_expected_scores(hg, ag, hv, av, odds_implied):
    ...
    if has_stats:
        ...
        # Pace: iki takım da yüksek tempolu (>108) → toplam baremi yukarı
        home_pace = hg.get("pace", LEAGUE_AVG / 2)
        away_pace = ag.get("pace", LEAGUE_AVG / 2)
        avg_pace  = (home_pace + away_pace) / 2
        pace_adj  = (avg_pace - (LEAGUE_AVG / 2)) * 0.5   # her 1 puan fark → 0.5 sayı etki

        # Efficiency: 100 poz başına yüksek üretim → ek skor
        home_eff = hg.get("efficiency", 100.0)
        away_eff = ag.get("efficiency", 100.0)
        avg_eff  = (home_eff + away_eff) / 2
        # Savunma verimliliği lig ortalamasının altındaysa (yani zayıf savunma) → +2.5
        eff_adj  = 2.5 if avg_eff > 105.0 else 0.0

        stat_total  = stat_total  + pace_adj + eff_adj
        # IY için aynı ayarın IY_RATIO kadarını uygula
...
def compute_probabilities(exp, home_var, away_var, market_ou_lines=None, market_iy_ou_lines=None):
    ...
    # Basketbolda toplam std ~ %6-8 oranında (literatür)
    gaussian_std_total = max(STD_TOTAL, ref_total * 0.07)

    if market_center and market_center > 160:
        scale     = market_center / LEAGUE_AVG
        std_total = max(gaussian_std_total * scale, std_diff * 1.1)
        std_iy    = max(7 * scale * IY_RATIO, std_total * IY_RATIO)
    else:
        std_total = max(gaussian_std_total, std_diff * 1.1)
