...
# ════════════════════════════════════════════════════════════════
# OU ANALİZİ — Bet365 line vs Model farkı sinyali
# ════════════════════════════════════════════════════════════════

# Bet365 line ile model tahmini arasındaki fark eşiği
# Bu farkın üzerinde → güçlü sinyal
OU_SIGNAL_THRESHOLD  = 8.0   # MS toplam için
IY_SIGNAL_THRESHOLD  = 4.0   # IY toplam için


def _ou_analysis(exp_total, std, lines, signal_threshold=OU_SIGNAL_THRESHOLD):
    """
    Her Bet365 line için:
    1. Model olasılığı (normal dağılım)
    2. Bet365 line - Model tahmin farkı → büyükse güçlü sinyal
    3. Fark > threshold → value_bet işaretle
    """
    result = []
    for l in lines:
        line = l["line"]
        ...
        # ── Bet365 vs Model fark sinyali ──────────────────────────
        # gap > 0  → Bet365 line yüksek, model düşük tahmin → ALT sinyali
        # gap < 0  → Bet365 line düşük, model yüksek tahmin → ÜST sinyali
        gap = line - exp_total
        ...
        if abs(gap) >= signal_threshold:
            if gap > 0:
                # Bet365 yüksek satıyor, model ALT diyor → ALT oyna
                vbet = "under"
                signal_strength = round(gap, 1)
            else:
                # Bet365 düşük satıyor, model ÜST diyor → ÜST oyna
                vbet = "over"
                signal_strength = round(abs(gap), 1)
        elif v_ov is not None and v_ov >= VALUE_THRESHOLD:
            vbet = "over"
        elif v_un is not None and v_un >= VALUE_THRESHOLD:
            vbet = "under"
        ...
        result.append({
            ...
            "odd_under":      un,
            "mkt_impl_over":  mi_ov,
            "mkt_impl_under": mi_un,
            "val_over":       v_ov,
            "val_under":      v_un,
            "value_bet":      vbet,
            "gap":            round(gap, 1),        # Bet365 - Model farkı
            "signal_strength": signal_strength,     # Fark büyüklüğü
        })
    return result


def _ou_auto(exp_total, std):
    """Bet365 line yoksa exp_total etrafında otomatik barem."""
...
def run_value_hunting(home_general, home_venue, away_general, away_venue,
    ...
    # MS Alt/Üst — Bet365 line varsa gap sinyali kullan
    ou_mkt   = mo.get("ou_lines_market", [])
    ou_lines = _ou_analysis(exp_total, std_total, ou_mkt, OU_SIGNAL_THRESHOLD) if ou_mkt else _ou_auto(exp_total, std_total)

    # IY Alt/Üst — daha küçük eşik
    iy_mkt      = mo.get("iy_ou_lines_market", [])
    iy_ou_lines = _ou_analysis(exp_iy_total, std_iy, iy_mkt, IY_SIGNAL_THRESHOLD) if iy_mkt else _ou_auto(exp_iy_total, std_iy)
