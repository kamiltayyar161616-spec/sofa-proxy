...
# Kupa / istenmeyen lig filtreleri
EXCLUDE_KW = [
...
# İzin verilen lig listesi
ALLOWED_LEAGUES = {
    "euroleague": True, "eurocup": True, "fiba": True,
    "basketball champions league": True,
    "bbl": True, "pro a": True, "lnb pro a": True,
    "pro b": True, "lega basket": True, "lba": True,
    "liga acb": True, "acb": True, "liga endesa": True,
    "bsn": True, "bnxt": True, "vbt": True, "beko bbl": True,
    "ebl": True, "nbl": True, "basketliiga": True,
...
def get_odds(event_key):
    ...
    try:
        for market in result:
            ...
            # En güvenilir bookmaker: bet365 > Marathon > Pncl > ilk mevcut
            bm_data = None
            for preferred in ["bet365", "Marathon", "Pncl", "1xBet", "Betano"]:
                for bm in bookmakers:
                    if preferred.lower() in (bm.get("bookmaker_name","")).lower():
                        bm_data = bm
                        break
                if bm_data:
                    break
            ...
            # ── 3-Way (MS) ──
            if "3way" in market_name.lower() or "result" in market_name.lower():
                oh = od = oa = None
                for o in odds_raw:
                    n = o.get("odd_name","").lower()
                    v = _safe_float(o.get("odd_value"))
                    if n in ("home","1"):    oh = v
                    elif n in ("draw","x"): od = v
                    elif n in ("away","2"): oa = v
                if oh: odds_out["odd_home"] = oh
                if od: odds_out["odd_draw"] = od
                if oa: odds_out["odd_away"] = oa
            ...
            # ── Asian Handicap ──
                ...
                for o in odds_raw:
                    ...
                    if not v:
                        continue
                    # odd_name örn: "Home" / "Away" fakat handicap değeri market_name içinde
                    # AllSports'ta her satır bir handicap değeri — odd_name = "Home" veya "Away"
                    # handicap değeri odd_name içinde olabilir örn: "-3.5" veya market_name suffix
                    hval = _safe_float(n)
                    side = "home" if "home" in n.lower() else ("away" if "away" in n.lower() else None)
                    if hval is not None and side:
                        key = round(hval, 1)
                ...
                if ah_lines:
                    ...
                    # En yakın sıfıra olan çizgiyi ana handikap olarak al
            ...
            # ── Over/Under (Toplam) ──
            elif "over/under" in market_name.lower() and "1st half" not in market_name.lower():
                ou_lines = {}
                for o in odds_raw:
                    n = o.get("odd_name","")
                    v = _safe_float(o.get("odd_value"))
                    if not v:
                        continue
                    # odd_name örn: "Over 170.5" veya "Under 170.5"
                    parts = n.strip().split()
                    if len(parts) == 2:
                        side = parts[0].lower()
                        line = _safe_float(parts[1])
                        if line and side in ("over","under"):
                            key = round(line, 1)
                ...
                if ou_lines:
                    odds_out["ou_lines_market"] = _build_ou_list(ou_lines)

            # ── Over/Under 1st Half (IY) ──
            elif "over/under" in market_name.lower() and "1st half" in market_name.lower():
                iy_lines = {}
                for o in odds_raw:
                    n = o.get("odd_name","")
                    v = _safe_float(o.get("odd_value"))
                    ...
                    if len(parts) == 2:
                        ...
                        if line and side in ("over","under"):
                            key = round(line, 1)
                            if key not in iy_lines:
                                iy_lines[key] = {}
                            iy_lines[key][side] = v

                if iy_lines:
                    odds_out["iy_ou_lines_market"] = _build_ou_list(iy_lines)

    except Exception as e:
        print(f"[Odds] Parse hatasi: {e}")
...
def _build_ou_list(ou_dict):
    """Over/Under dict'ini sıralı listeye çevir."""
    result = []
    for line in sorted(ou_dict.keys()):
        entry = ou_dict[line]
        result.append({
            "line":      line,
            "odd_over":  entry.get("over"),
            "odd_under": entry.get("under"),
        })
    return result
...
def get_match_data(home_id, away_id, league_key=None):
    ...
    return {
        "home_general": home_all[:10],
        "home_venue":   home_venue[:6],
        "away_general": away_all[:10],
