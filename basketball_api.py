...
try:
    ...
    SOFA_ENABLED = False
    print("[BasketAPI] sofa_odds modülü yok, odds devre dışı")
...
def get_fixtures_by_date(date_str):
    ...
    for ev in all_results:
        try:
            ...
            # SofaScore'dan market oranlarını çek
            odds = {}
            if SOFA_ENABLED:
                try:
                    odds = sofa.get_odds(home_team, away_team, date_str)
                except Exception as oe:
                    print(f"[BasketAPI] Sofa odds hata: {oe}")
            ...
            matches.append({
                ...
                # SofaScore market oranları
                "mkt_ms_home":    odds.get("ms_home"),
                "mkt_ms_away":    odds.get("ms_away"),
                "mkt_ou_line":    odds.get("ou_line"),
                "mkt_ou_over":    odds.get("ou_over"),
                "mkt_ou_under":   odds.get("ou_under"),
                "mkt_iy_line":    odds.get("iy_line"),
                "mkt_iy_over":    odds.get("iy_over"),
                "mkt_iy_under":   odds.get("iy_under"),
                "mkt_spread_line":odds.get("spread_line"),
                "mkt_spread_home":odds.get("spread_home"),
                "mkt_spread_away":odds.get("spread_away"),
                "sofa_found":     odds.get("found", False),
            })
