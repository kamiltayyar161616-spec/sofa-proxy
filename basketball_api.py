...
API_KEY   = os.environ.get("ALLSPORTS_BASKET_KEY", "552d02afa498d630efe42a89500cb1964dcba1b23fd9a929dc526fc3b1523747")
...
def get_odds(event_key):
    """
    AllSports API'den maç oranlarını tek seferde çeker.

    Bookmaker önceliği: bet365 > Marathon > 1xBet > Pncl > Sbo > ilk bulunan
    Her market için tek barem seçilir:
      - Handikap : oranları %50/%50'ye en yakın barem
      - Alt/Üst  : oranları %50/%50'ye en yakın barem
      - İY Alt/Üst: oranları %50/%50'ye en yakın barem
      - MS       : doğrudan 1 / X / 2

    Döner:
    {
        "odd_home":    1.70,   "odd_draw": 13.5,  "odd_away": 2.17,
        "ah_line":    -3.5,   "ah_home":  1.88,  "ah_away":  1.88,
        "ou_line":   170.5,   "ou_over":  1.85,  "ou_under": 1.85,
        "iy_ou_line": 87.5,   "iy_over":  1.85,  "iy_under": 1.85,
        # Eski alan adları (app.py uyumu)
        "odd_home", "odd_draw", "odd_away",
        "ah_main_line", "ah_main_home", "ah_main_away",
        "ou_lines_market":    [{"line":..., "odd_over":..., "odd_under":...}],
        "iy_ou_lines_market": [{"line":..., "odd_over":..., "odd_under":...}],
    }
    """
    ek = str(event_key)
    ...
    if not result:
        print(f"[Odds] eventId={ek} veri yok")
        _odds_cache[ek] = {}
        return {}
    ...
    if isinstance(raw, dict):
        keys = list(raw.keys())
        if keys and str(keys[0]).isdigit():   # {"41223": {markets}} wrapper
            raw = raw[keys[0]]
    ...
    # ── Bookmaker önceliği ───────────────────────────────────────
    # bet365 > Marathon > 1xBet > Pncl > Sbo > ilk bulunan
    BM_PRIORITY = ["bet365", "marathon", "1xbet", "pncl", "sbo",
                   "betano", "williamhill", "unibet", "bwin", "188bet"]

    def pick_val(bm_dict):
        """Bookmaker dict'inden öncelik sırasına göre değer al."""
        if not isinstance(bm_dict, dict):
            return None
        for bm in BM_PRIORITY:
            for k, v in bm_dict.items():
                if bm in k.lower():
                    f = _safe_float(v)
                    if f and f > 1.0:
                        return f
        # Hiçbiri bulunamazsa ilk geçerli değeri al
        for v in bm_dict.values():
            f = _safe_float(v)
            if f and f > 1.0:
                return f
    ...
    def balanced_line(lines_dict):
        """
        {line_val: {"over/home": odd, "under/away": odd}} yapısından
        oranları %50/%50'ye en yakın (dengeli) baremi seç.
        Denge skoru = |odd1 - odd2| → en küçük = en dengeli
        """
        best_line = None
        best_diff = float("inf")
    ...
    # ── Market verileri topla ────────────────────────────────────
    ah_lines = {}   # {line_val: {"home": odd, "away": odd}}
    ou_lines = {}   # {line_val: {"over": odd, "under": odd}}
    iy_lines = {}   # {line_val: {"over": odd, "under": odd}}
    odds_out = {}
    ...
    try:
        for market_name, sides in raw.items():
            ...
            # ── MS (3-Way) ───────────────────────────────────────
            if "3way" in mlow or ("result" in mlow and "asian" not in mlow
                                   and "1st" not in mlow and "half" not in mlow):
                for side_name, bm_dict in sides.items():
                    val = pick_val(bm_dict)
                    if not val:
                        continue
            ...
            # ── Asian Handicap (maç) ─────────────────────────────
                ...
                line_val = extract_num(market_name)
                if line_val is None:
                    continue
                for side_name, bm_dict in sides.items():
                    val = pick_val(bm_dict)
    ...
    # ── Dengeli handikap baremi seç ──────────────────────────────
    complete_ah = {k: v for k, v in ah_lines.items()
                   if "home" in v and "away" in v}
    if complete_ah:
        best = balanced_line(complete_ah)
        odds_out["ah_line"]      = best
        odds_out["ah_home"]      = complete_ah[best]["home"]
        odds_out["ah_away"]      = complete_ah[best]["away"]
        # Eski alan adları (app.py uyumu)
        odds_out["ah_main_line"] = best
        odds_out["ah_main_home"] = complete_ah[best]["home"]
        odds_out["ah_main_away"] = complete_ah[best]["away"]

    # ── Dengeli Alt/Üst baremi seç ───────────────────────────────
    complete_ou = {k: v for k, v in ou_lines.items()
                   if "over" in v and "under" in v}
    if complete_ou:
        best = balanced_line(complete_ou)
        odds_out["ou_line"]   = best
        odds_out["ou_over"]   = complete_ou[best]["over"]
        odds_out["ou_under"]  = complete_ou[best]["under"]
        # app.py uyumu: liste formatında da ver (tek elemanlı)
        odds_out["ou_lines_market"] = [
            {"line": best, "odd_over": complete_ou[best]["over"],
             "odd_under": complete_ou[best]["under"]}
        ]

    # ── Dengeli İY Alt/Üst baremi seç ───────────────────────────
    complete_iy = {k: v for k, v in iy_lines.items()
                   if "over" in v and "under" in v}
    if complete_iy:
        best = balanced_line(complete_iy)
        odds_out["iy_ou_line"]  = best
        odds_out["iy_over"]     = complete_iy[best]["over"]
        odds_out["iy_under"]    = complete_iy[best]["under"]
        # app.py uyumu
        odds_out["iy_ou_lines_market"] = [
            {"line": best, "odd_over": complete_iy[best]["over"],
             "odd_under": complete_iy[best]["under"]}
        ]

    if odds_out:
        print(f"[Odds] eventId={ek} → "
              f"MS:{odds_out.get('odd_home')}/{odds_out.get('odd_away')} | "
              f"AH:{odds_out.get('ah_line')} | "
              f"OU:{odds_out.get('ou_line')} | "
              f"IY-OU:{odds_out.get('iy_ou_line')}")
    else:
...
def get_fixtures_by_date(date_str):
    ...
    for ev in all_results:
        try:
            ...
            # Odds artık analyze endpoint'te çekiliyor
            odds = {}
