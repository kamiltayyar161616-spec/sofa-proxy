...
def _parse_odds(ev):
    ...
    Basketbol API'si farklı key formatları kullanabilir:
    - "Over/Under 214.5"
    ...
    # ── MS Over/Under — tüm olası formatları tara ───────────────
    ou_lines = []
    for key, val in odds_raw.items():
        if not isinstance(val, dict):
            continue
        # İY çizgilerini atla
        if any(x in key.lower() for x in ["1st half", "half", "quarter", "iy", "halftime"]):
            continue
        # Sayısal değer içeren OU key'lerini yakala
        line = None
        key_lower = key.lower()
        for prefix in ["over/under ", "total over/under ", "totals ", "asian total ",
                        "o/u ", "total points ", "total "]:
            if key_lower.startswith(prefix):
                try:
                    line = float(key[len(prefix):].strip())
                    break
                except Exception:
                    pass
        if line is None:
    ...
    for key, val in odds_raw.items():
        if not isinstance(val, dict):
            continue
        key_lower = key.lower()
        is_iy = any(x in key_lower for x in ["1st half", "halftime", "half time"])
        if not is_iy:
            continue
        line = None
        for prefix in ["over/under 1st half ", "1st half over/under ",
            ...
            if key_lower.startswith(prefix):
                try:
                    ...
                    pass
        if line is None:
            # "Over/Under 1st Half 110.5" gibi içinde sayı olan formatlar
            import re
