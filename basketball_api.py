...
def _parse_odds_from_fixture(ev):
    """
    Fikstür event objesinin içindeki 'odds' alanını parse et.
    Yapı: ev["odds"]["3Way Result"]["Home"]["Marathon"] = "1.70"
    """
    odds_raw = ev.get("odds")
    if not odds_raw or not isinstance(odds_raw, dict):
        return {}
    # Aynı get_odds mantığını kullan
    # Geçici event_key atayıp get_odds_from_dict çağır
    return _parse_odds_dict(odds_raw)


def _parse_odds_dict(raw):
    """Ham odds dict'ini parse et — get_odds ve fixture parse için ortak."""
    odds_out = {}
    ah_lines = {}
    ou_lines = {}
    iy_lines = {}
    ...
    for market_name, sides in raw.items():
        ...
        if "3way" in mlow or ("result" in mlow and "asian" not in mlow and "1st" not in mlow):
            for side_name, bm_dict in sides.items():
                ...
                if not val:
                    continue
                if   slow in ("home", "1"): odds_out["odd_home"] = val
                elif slow in ("draw", "x"): odds_out["odd_draw"] = val
                elif slow in ("away", "2"): odds_out["odd_away"] = val

        # Asian Handicap
        elif "asian handicap" in mlow and "1st" not in mlow:
            nums = re.findall(r'-?\d+\.?\d*', market_name)
            line = None
            for n in nums:
                try:
                    v = float(n)
            ...
            entry = ah_lines.setdefault(line, {})
            for side_name, bm_dict in sides.items():
                slow = side_name.lower()
                val  = _pick_bm_value(bm_dict)
                if val:
                    if   slow in ("home","1"): entry["home"] = val
                    elif slow in ("away","2"): entry["away"] = val

        # MS Over/Under
        elif market_name.lower().startswith("over/under") and "1st" not in market_name.lower():
            nums = re.findall(r'\d+\.?\d*', market_name)
            if not nums: continue
            line = float(nums[0])
            entry = ou_lines.setdefault(line, {})
            for side_name, bm_dict in sides.items():
                slow = side_name.lower()
                val  = _pick_bm_value(bm_dict)
                if val:
                    if   "over"  in slow: entry["over"]  = val
                    elif "under" in slow: entry["under"] = val

        # IY Over/Under
        elif "1st half" in market_name.lower() and "over" in market_name.lower():
            nums = re.findall(r'\d+\.?\d*', market_name)
            if not nums: continue
            line = float(nums[0])
            entry = iy_lines.setdefault(line, {})
            for side_name, bm_dict in sides.items():
                slow = side_name.lower()
                val  = _pick_bm_value(bm_dict)
                if val:
    ...
    if ou_lines:
        odds_out["ou_lines_market"] = [
            {"line": k, "odd_over": v.get("over"), "odd_under": v.get("under")}
    ...
    if iy_lines:
        odds_out["iy_ou_lines_market"] = [
            {"line": k, "odd_over": v.get("over"), "odd_under": v.get("under")}
            for k, v in sorted(iy_lines.items())
            if v.get("over") and v.get("under")
        ]
...
def get_odds(event_key):
    ...
    result = _get({"met": "Odds", "matchId": ek})
    if not result:
...
def get_fixtures_by_date(date_str):
    ...
    for ev in all_results:
        try:
            ...
            # Önce fikstür içinden parse et, boşsa API çek
            odds = _parse_odds_from_fixture(ev)
            if not odds.get("odd_home") and not odds.get("ou_lines_market"):
                odds = get_odds(mid)
