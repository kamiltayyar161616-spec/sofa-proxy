...
def api_fixtures():
    ...
    try:
        ...
        traceback.print_exc()
        return jsonify({"success": False, "matches": [], "error": str(e)}), 200
...
def api_analyze(home_id, away_id):
    ...
    try:
        ...
        # ── Odds: match_id ile API'den tamamla ────────────────────
        ...
        if oapi:
            try:
                ...
                if hn and an:
                    bet365 = oapi.get_market_odds(hn, an, dt)
            except Exception as be:
                print(f"[bet365] hata: {be}")

        # ── run_value_hunting — tüm odds toplandıktan sonra ────────
...
def api_clear_cache():
    ...
    if oapi: oapi.clear_cache()
    return jsonify({"success": True, "message": "Cache temizlendi"})



@app.route("/api/debug-bet365")
def api_debug_bet365():
    """Bet365 API ham verisini göster — takım ismi formatını bul."""
    import requests as req
    dt = request.args.get("dt", today_str())
    key = os.environ.get("ODDS_API_KEY", "e097279bf7409b014ed0b9bdff8df8d8ce4045b2e335542d9cde58e0c3462a42")
    try:
        r = req.get(
            "https://api.odds-api.io/v3/events",
            params={"sport": "Basketball", "date": dt, "bookmakers": "Bet365", "apiKey": key},
            timeout=10
        )
        data = r.json()
        # Sadece ilk 20 eventi göster
        events = data if isinstance(data, list) else data.get("data", data.get("events", []))
        sample = [{"id": e.get("id"), "home": e.get("home", e.get("home_team","")), "away": e.get("away", e.get("away_team",""))} for e in events[:20]]
        return jsonify({"status": r.status_code, "count": len(events) if events else 0, "sample": sample, "raw_keys": list(data.keys()) if isinstance(data, dict) else "list"})
    except Exception as e:
