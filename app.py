...
def api_debug_bet365():
    """Bet365 API ham event listesini göster."""
    dt = request.args.get("dt", today_str())
    if not oapi:
        return jsonify({"error": "oapi not loaded"})
    try:
        events = oapi.get_all_events(dt)
        sample = [{"id": e.get("id"), "home": e.get("home", e.get("home_team","")),
                   "away": e.get("away", e.get("away_team",""))} for e in events[:30]]
