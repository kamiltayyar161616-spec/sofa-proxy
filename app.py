...
def api_debug_odds():
    """Ham API odds yapısını görmek için - hangi key formatı geliyor?"""
    date = request.args.get("date", today_str())
    import requests as req
    try:
        r = req.get(
            "https://apiv2.allsportsapi.com/basketball/",
            params={"met": "Fixtures", "APIkey": bapi.API_KEY, "from": date, "to": date},
            headers=bapi.HEADERS, timeout=15
        )
        data = r.json()
        results = data.get("result", [])
        # İlk 3 maçın odds key'lerini döndür
        odds_samples = []
        for ev in results[:5]:
            odds = ev.get("odds", {})
            if odds:
                odds_samples.append({
                    ...
                    "full_odds": odds  # ham yapı
                })
        return jsonify({"date": date, "total_matches": len(results), "samples": odds_samples})
    except Exception as e:
