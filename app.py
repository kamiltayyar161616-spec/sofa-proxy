...
def api_debug_odds():
    """
    Ham API odds yapısını görmek için.
    Her maç için:
      - Fixture içindeki odds key'leri
      - met=Odds ile ayrı çekilen key'ler
      - Her ikisinin parse sonucu
    Kullanım: /api/debug-odds  veya  /api/debug-odds?date=2025-01-15
    """
    date = request.args.get("date", today_str())
    try:
        samples = bapi.get_raw_odds_sample(date)
        return jsonify({
            "date":    date,
            "count":   len(samples),
            "samples": samples,
...
@app.route("/api/debug-odds-match/<int:match_id>")
def api_debug_odds_match(match_id):
    """Tek bir maç için met=Odds ham verisini döndür."""
    import requests as req
    try:
        r = req.get(
            "https://apiv2.allsportsapi.com/basketball/",
            params={"met": "Odds", "APIkey": bapi.API_KEY, "matchId": match_id},
            headers=bapi.HEADERS, timeout=15
        )
        data = r.json()
        return jsonify(data)
    except Exception as e:
