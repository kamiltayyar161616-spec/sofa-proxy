...
def api_analyze(home_id, away_id):
    ...
    try:
        ...
        if not data["home_general"] and not data["away_general"]:
            fb = vh.fallback_result()
            if market_odds:
                fb["market_odd_home"] = odd_home
                fb["market_odd_draw"] = odd_draw
                fb["market_odd_away"] = odd_away
            # league_key yoksa bunu logla
            if league_key is None:
                print(f"[analyze] league_key yok: home={home_id} away={away_id}, fallback kullanılıyor")
            return jsonify({"success": True, "analysis": fb})
