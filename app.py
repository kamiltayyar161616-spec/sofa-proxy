...
def api_analyze(home_id, away_id):
    ...
    try:
        ...
        # ── Odds: query string'den al (fixture listesinden gelir) ──
...
def api_debug_fixtures():
    """Ham API sonucunu göster — sorun tespiti için."""
    date = request.args.get("date", today_str())
    import basketball_api as bapi2
    import datetime as dt2

    try:
        tr_day    = dt2.datetime.strptime(date, "%Y-%m-%d")
        prev_date = (tr_day - dt2.timedelta(days=1)).strftime("%Y-%m-%d")
        next_date = (tr_day + dt2.timedelta(days=1)).strftime("%Y-%m-%d")
    except Exception:
        prev_date = next_date = date

    raw = {}
    for d in [prev_date, date, next_date]:
        r = bapi2._get({"met": "Fixtures", "from": d, "to": d})
        raw[d] = len(r) if r else 0

    live = bapi2._get({"met": "Livescore"})
    sample = []
    if live:
        for ev in live[:3]:
            sample.append({
    ...
    matches = bapi2.get_fixtures_by_date(date)
    return jsonify({
        "date": date,
        "raw_counts": raw,
