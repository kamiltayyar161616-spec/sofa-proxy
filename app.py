...
def api_debug_sofa():
    ...
    an = request.args.get("an", "")
    if not oapi:
        return jsonify({"error": "sofa_odds not loaded"})
    try:
        events = oapi.load_events(dt)
        sample = [{"id": e.get("id"),
                   "home": e.get("homeTeam", {}).get("name", ""),
                   "away": e.get("awayTeam", {}).get("name", "")} for e in events[:30]]
        result = {"count": len(events), "sample": sample}
        if hn and an:
            odds = oapi.get_odds(hn, an, dt)
            result["odds"] = odds
