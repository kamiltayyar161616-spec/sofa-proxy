...
def api_sofa_events():
    """SofaScore basketbol eventlerini JSON dosyası olarak indir."""
    ...
    try:
        r = req.get(
            f"https://api.sofascore.com/api/v1/sport/basketball/scheduled-events/{dt}",
            headers=headers, timeout=15
        )
        data = r.json()
        events = data.get("events", [])
        # Sadece lazım olan alanları al
        simplified = []
        for ev in events:
            simplified.append({
                "id": ev.get("id"),
        ...
        resp = Response(
            _json.dumps(simplified, ensure_ascii=False, indent=2),
            mimetype="application/json",
            headers={"Content-Disposition": f"attachment; filename=sofa_{dt}.json"}
        )
        return resp
    except Exception as e:
