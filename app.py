...
def _inject_ou(result, ou_json, iyou_json):
    """Frontend'den gelen OU oranlarını analiz sonucuna ekle."""
    import json as _json
    try:
        if ou_json:
            ou = _json.loads(ou_json)
            if ou:
                best = ou[0]
                result["mkt_ou_line"]  = best.get("line")
                result["mkt_ou_over"]  = best.get("odd_over")
                result["mkt_ou_under"] = best.get("odd_under")
                # Value hesapla
                if result.get("exp_total") and best.get("odd_over") and best.get("line"):
                    import math
                    std = 12.0 * math.sqrt(2)
                    z   = (best["line"] - result["exp_total"]) / std
                    p   = 1 - (0.5*(1+math.erf(z/math.sqrt(2))))
                    val_ov = round((p*best["odd_over"]-1)*100, 1)
                    val_un = round(((1-p)*best.get("odd_under",best["odd_over"])-1)*100, 1)
                    result["val_ou_over"]  = val_ov
                    result["val_ou_under"] = val_un
                    vb = result.get("value_bets", [])
                    if val_ov > 5:
                        vb.append(f"Alt/Üst {best['line']} ÜST @ {best['odd_over']} → +{val_ov}% VALUE")
                    elif val_un > 5:
                        vb.append(f"Alt/Üst {best['line']} ALT @ {best['odd_under']} → +{val_un}% VALUE")
                    result["value_bets"] = vb
    except: pass
    try:
        if iyou_json:
            iyou = _json.loads(iyou_json)
            if iyou:
                best = iyou[0]
                result["mkt_iy_line"]  = best.get("line")
                result["mkt_iy_over"]  = best.get("odd_over")
                result["mkt_iy_under"] = best.get("odd_under")
    except: pass
...
def api_analyze(home_id, away_id):
    ...
    home_team  = request.args.get("hn", "") or request.args.get("ht", "")
    away_team  = request.args.get("an", "") or request.args.get("at", "")
    match_date = request.args.get("dt", today_str())
    odd_home   = request.args.get("oh", type=float)
    odd_away   = request.args.get("oa", type=float)
    ou_json    = request.args.get("ou", "")
    iyou_json  = request.args.get("iyou", "")
    cache_key  = f"ana_{home_id}_{away_id}_{match_date}"

    # Cache'de varsa hemen dön
    cached = _cget(cache_key)
    if cached:
        # Gelen market oranları varsa cache'i güncelle
        if odd_home and not cached.get("mkt_ms_home"):
            cached["mkt_ms_home"] = odd_home
            cached["mkt_ms_away"] = odd_away
            _inject_ou(cached, ou_json, iyou_json)
            _cset(cache_key, cached, 21600)
    ...
    try:
        ...
        if not result:
            result = vh.fallback_result()
        # Market oranlarını ekle
        if odd_home:
            result["mkt_ms_home"] = odd_home
            result["mkt_ms_away"] = odd_away
            result["fair_ms_home"] = round(100/result["prob_home"], 2) if result.get("prob_home") else None
            result["fair_ms_away"] = round(100/result["prob_away"], 2) if result.get("prob_away") else None
            if result.get("prob_home") and odd_home:
                result["val_ms_home"] = round((result["prob_home"]/100*odd_home-1)*100, 1)
            if result.get("prob_away") and odd_away:
                result["val_ms_away"] = round((result["prob_away"]/100*odd_away-1)*100, 1)
        _inject_ou(result, ou_json, iyou_json)
