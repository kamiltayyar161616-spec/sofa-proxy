        ...
        if oapi:
            try:
                ...
                if hn and an:
                    ...
                    # Bet365'ten gelen OU line'larını market_odds'a ekle
                    if bet365.get("found"):
                        if not oh and bet365.get("ms_home"):  oh = bet365["ms_home"]
                        if not oa and bet365.get("ms_away"):  oa = bet365["ms_away"]
                        # MS OU — Bet365 tek line
                        if not ou_market and bet365.get("ou_line"):
                            ou_market = [{
                                "line":      bet365["ou_line"],
                                "odd_over":  bet365.get("ou_over"),
                                "odd_under": bet365.get("ou_under"),
                            }]
                        # IY OU — Bet365 tek line
                        if not iy_ou_market and bet365.get("iy_line"):
                            iy_ou_market = [{
                                "line":      bet365["iy_line"],
                                "odd_over":  bet365.get("iy_over"),
                                "odd_under": bet365.get("iy_under"),
                            }]
